...
async function batchUpdate(tableId, updates, errors) {
  const seen = new Set();
  const deduped = [];
    ...
    try {
      ...
      for (const rec of slice) {
        try {
          ...
          // FIX (silent save failures): this used to only console.warn — the
          // record was skipped, handleUpdate carried on, and the function
          // returned 200 {ok:true}. The client then showed "Saved ✓" even
          // though this specific record was never written. Now the failure
          // is collected and surfaced in the response so the client can
          // detect it, warn instead of claiming success, and retry.
          console.warn(`[SELECT API] batchUpdate: skipped record ${tableId}/${rec.id}:`, e2.message);
          if (errors) errors.push({ table: tableId, id: rec.id, action: "update", message: e2.message });
        }
      }
    }
  }
...
async function batchDelete(tableId, ids, errors) {
    ...
    try {
      ...
      for (const id of batch) {
        try {
          ...
          if (errors) errors.push({ table: tableId, id: id, action: "delete", message: e2.message });
        }
      }
    }
  }
...
async function handleUpdate(payload) {
  ...
  // FIX (silent save failures): collects real per-record Airtable write
  // failures from every batchUpdate/batchDelete call below. Previously these
  // were only console.warn'd inside batchUpdate/batchDelete and this function
  // always returned 200 {ok:true} regardless, so a rejected write (rate limit,
  // transient Airtable error, etc.) on any individual row was invisible to the
  // client — the UI showed "Saved ✓" while the record silently kept its old
  // value. Now any such failure is reported back so the client can retry.
  const _rowErrors = [];
  ...
  if (Array.isArray(progress) || Array.isArray(notes)) {
    ...
    const toDelete = existingNotes
      ...
      .map((r) => r.id);
    await batchDelete(TABLES.NOTES, toDelete, _rowErrors);
  ...
  // client. Status stays 200 (the request itself was handled; most of the
  // payload very likely did save) but ok:false + errors lets the client stop
  // reporting "Saved ✓" and instead keep the org queued for retry.
  if (_rowErrors.length) {
    console.error(`[SELECT API] handleUpdate: ${code} completed with ${_rowErrors.length} row-level failure(s):`, JSON.stringify(_rowErrors));
    return jsonResponse(200, { ok: false, errors: _rowErrors });
  }
  return jsonResponse(200, { ok: true });
}
